...
export const getChainRegContents = () => {
  ...
  if (chain) {
    ...
    const filter4 = (channel: IbcChannel) =>
      (
        channel.property(IbcChannel.CHAIN_1)[IbcChannel.CHANNEL_ID] === "channel-0" || 
        channel.property(IbcChannel.CHAIN_2)[IbcChannel.CHANNEL_ID] === "channel-0"
      ) &&
      (
        channel.property(IbcChannel.CHAIN_1)[IbcChannel.PORT_ID] === "transfer" ||
        channel.property(IbcChannel.CHAIN_2)[IbcChannel.PORT_ID] === "transfer"
      )// &&
      //channel.property(IbcChannel.TAGS)?.status === "live";
      //channel.property(IbcChannel.CHAIN_1)?.port_id === "transfer"; 

    console.log(chain_reg.ibcConnection("osmosis", "cosmoshub")?.channels([filter4]));
