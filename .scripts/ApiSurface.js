...
function extractTypeScriptSurface(filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if ((isInterface || isType) && !isClass) {
            ...
            // If the declaration doesn't immediately close on the same line
            if (!(line.endsWith(';') && braceDepth <= 0) && (line.includes('{') || line.includes('='))) {
                let j = i + 1;
                while (j < lines.length) {
                    ...
                    // FIXED: Only break on a semicolon if we aren't trapped inside unclosed braces
                    if ((subLine.endsWith(';') && braceDepth <= 0) || (line.includes('{') && braceDepth <= 0)) {
                        i = j;
                        break;
                    }
                    j++;
        ...
        if (isClass) {
            ...
            // FIXED: Ensure the opening brace is preserved or appended nicely for structural layout
            ...
            for (let k = 0; k < classBodyLines.length; k++) {
                ...
                // Optional: Clean up messy leading cross-line union pipes after colons
                cleanedSig = cleanedSig.replace(/:\s*\|\s*/, ': ');
